...
- UI-Button-Handler, die mehrere Formularfelder auf einmal ändern (z. B. `SelectAllEntities`, `DeselectAllEntities`), schreiben niemals per `IPS_SetProperty` + `IPS_ApplyChanges` direkt in die Properties. Stattdessen werden alle geänderten Felder via `UpdateFormField` aktualisiert, damit der Benutzer die Vorschau der Änderungen sieht und eigene, noch nicht gespeicherte Anpassungen nicht verloren gehen. Programmatische API-Methoden (z. B. `ActivateBundleMode`, `ActivateMqttMode`), die für den Aufruf aus Scripts oder Automation gedacht sind, dürfen hingegen `IPS_SetProperty` + `IPS_ApplyChanges` verwenden, da hier keine interaktive Bestätigung durch den Benutzer stattfindet.
- Aktionen auf Symcon-Variablen werden ausschließlich über `MaintainAction` oder `DisableAction` gesteuert. `IPS_SetVariableCustomAction` hat in Modulen nichts zu suchen — die benutzerdefinierte Aktion bleibt Sache des Benutzers.
- Variablen, die vom Modul selbst angelegt wurden, werden ausschließlich per `$this->UnregisterVariable($ident)` entfernt, nicht per `IPS_DeleteVariable`. Direktaufrufe von `IPS_DeleteVariable` auf eigene Variablen sind unzulässig.
