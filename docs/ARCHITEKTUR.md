...
- Button-Handler, die mehrere Formularfelder auf einmal ändern (z. B. `SelectAllEntities`, `ActivateBundleMode`), schreiben niemals per `IPS_SetProperty` + `IPS_ApplyChanges` direkt in die Properties. Stattdessen werden alle geänderten Felder via `UpdateFormField` aktualisiert, damit der Benutzer die Vorschau der Änderungen sieht und eigene, noch nicht gespeicherte Anpassungen nicht verloren gehen.
