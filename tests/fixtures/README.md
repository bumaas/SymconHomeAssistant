...
Versioniert sind nur inhaltlich geprueft unbedenkliche Bundles (`ebusd`, die beiden
`*light*`-Bundles sowie das Mitsubishi-Device-Config-Bundle). Alle uebrigen Bundles
bleiben lokal, da sie private Daten (Broker-IP, MQTT-Benutzername) enthalten —
neue Fixtures vor dem Einchecken entsprechend pruefen und in `.gitignore` freischalten.
...
- `ha_mqtt_discovery_bundle_zigbee2mqtt_light_current_v2.json`
  ...
  - Producer-Version: leer im Bundle
  - Zweck: fixture-nahe Parser-, Gruppierungs- und Runtime-Pruefung fuer Discovery-`light` mit aktuellem Cache-/Session-Stand
  - Enthaelt: `88` `light`-Discovery-Configs plus referenzierte Runtime-Topics fuer den dedizierten Light-Checker
  - Beobachtung: eignet sich als kompakte Referenz, um `schema=json`, `supported_color_modes`, `effect_list` und Light-Command-Payloads reproduzierbar zu pruefen
...
- `ha_mqtt_discovery_bundle_zigbee2mqtt_light_stale_v2.json`
  ...
  - Producer-Version: leer im Bundle
  - Zweck: fixture-nahe Light-Pruefung mit stale/current-Mix fuer Cache- und Diagnosefaelle
  - Enthaelt: denselben Light-Fokus mit abweichendem Session-Stand fuer Freshness-Checks
  - Beobachtung: eignet sich, um den Light-Pfad auch gegen stale Referenzen reproduzierbar zu pruefen
...
Der Light-Checker sammelt automatisch alle `*light*.json` unter `tests/fixtures` und prueft Light-Metadaten, Gruppierung, Runtime-State-Extraktion und Command-Payloads.
